/**
 * preprocess-svg.jsLogic for saving off an svg file that has been
 * preprocessed. It put a class in the svg file called "preprocessed".Created 6/18/2017 by Larry A. Maddocks
 */
//TODO: make the preprocessing part a functin that is avaiabl to both 
...
var version = "4. 3.01"; //made for pre-processing an svg file from preProcessSvg.js
...
var folderLocationOfFiles = null; //this is the location of the svg and json and html files for a particular map site
...
function svgloaded() {
  ...
  setTimeout(function() {
    ...
    if (!$(svgness).hasClass("preprocessed")) {
      ...
      //TODO: ********** save this svg file on server

    }
    //clone svg file
    //where send that svg file?
    //send in the svg file
    var svgFile = folderLocationOfFiles + "/" + lastEmbedSrc;
    var xmlCopy = $(svgness).clone()[0];

    var serializer = new XMLSerializer();
    var svgString = serializer.serializeToString(xmlCopy);

    var xml = svgString.replace(/(unicode=\".+\")/g, ''); //take out unicodes, some of which break the svg
    xml = '<?xml version="1.0" standalone="no"?>\r\n' + xml;

    var data = {};
    data.svgFile = svgFile;
    data.folderLocationOfFiles = folderLocationOfFiles;
    data.doc = JSON.stringify({
      xml: xml
    });
    $.post("/preProcessSvg/upload", data, function(result)  {
      if (result.status === "success") {

        console.log("uploaded preprocessed svg ");
        $("#serverMessage").html("Uploaded processed svg.");
      }
      else {
        $("#serverMessage").html("Something went wrong while trying to upload processed svg.");
        console.log("Failed to upload preprocessed svg ");
      }
...
function getDatabaseAndInfoFile(localVersion) {
  ...
  return; //we don't need following (deleted code) for the pre-processor
...
function main(designerprefsFileName) {
  ...
  folderLocationOfFiles = designerprefsFileName.substring(0, designerprefsFileName.lastIndexOf("/"));
  //$.getJSON(designerprefsFileName, function(j) {
  $.getJSON("config.json", function(j) {
      ...
      return idiagramSvg.lastUpdatedDb.getItem(idiagramSvg.designerPrefs.masterDB).
      then(function(value) {
    ...
    .done(function() {
      console.log("in idiagramSvg.lastUpdatedDb.iterate done");
      return null;
    })
    .fail(function() {
...
function purgeMatchedStatesFromParamList(parameterList) {
  ...
  for (var i = 0; i < parameterList.length; i++) {
    ...
    // hoover off
    var rfxtypes = /^(?:\+\+\+|panx|pany|zoom|panzoom|all|animate|play|stop|stopall|openall|closeall|wpane|epane|info|story|edit|map|infotext|storytext|maptext|goto|gotoz|run|move|scale|fade|rotate|masteron|masteroff)$/;
  ...
  if (svgness !== undefined) {
    ...
    if (!q || q.length < 2) {
      ...
      var keys = ["+++", "on", "off", "open", "close", "all", "animate", "play", "stop", "openall", "closeall", "stopall",
        "info", "story", "edit", "infotext", "maptext", "storytext", "map", "goto", "gotoz", "run", "move", "scale", "fade", "rotate",
        "masteron", "masteroff"
      ];
...
function createNewEmbed(src) {
  //if we don't have the folder location of the svg file, or not the svg file, bail
  if (!folderLocationOfFiles || !src) {
    console.error("Missing svg file and or location of svg file");
    return null;
  }
  var svgFile = folderLocationOfFiles + "/" + src;
...
function getDatabase(thisId, field) {
  ...
  else if (row !== undefined && row.id !== undefined && row[field] !== undefined) {
    return row[field];
  }
      ...
      case "edit": //edit map database. Looks like edit=unoid. Use this so you don't have to click on an uno to edit it.
        ...
        if ($("#mapDbEditor").length === 0) //don't reload template file if we already have it. Note: someday we may want to load different map db editors
        {
          $.get("/mapDbEditor.hbs", function(unoIdToEdit, fileContents, status) {
            ...
            mapDbApi.populateMapForm(unoIdToEdit);
