...
function getFormData2Object(j) {
     ...
     j = JSON.parse(j);
     for (var key in j) {
          var name = j[key].dbField;
          if (j[key].type === "radio-group" || j[key].type === "checkbox-group") {
               var values = j[key].values;
               for (var radioValues in values) {
                    //if (values[radioValues].value === idiagramSvg.getDatabase(data.id, name)) {
                    if (values[radioValues].selected == true) _array[name] = values[radioValues].value !== undefined ? values[radioValues].value : '';
                    // }
               }
          } else {
          ...
          if (result && result.hasOwnProperty("status") && result.status == "error") {
               ...
               for (var key in j) {
                    ...
                    if (j[key].type === "radio-group" || j[key].type === "checkbox-group") {
                         var values = j[key].values;
                         for (var radioValues in values) {
                              if (values[radioValues].value === RowCopy[name]) {
                                   //idiagramSvg.getDatabase(data.id, name)) {
...
function populateMapForm(formData, unoId) {
     ...
     var options = {
          dataType: 'json',
          formData: formData,
          disableFields: ['autocomplete', 'button', 'checkbox', 'checkbox-group', 'date', 'file', 'header', 'hidden', 'paragraph', 'number', 'radio-group', 'select', 'text', 'textarea'],
