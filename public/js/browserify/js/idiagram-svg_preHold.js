...
/* global $, 
    ...
    baseIdClass, svgFile, folder,storyFile,infoFile, masterDB, overrideDB,zoomSensitivity ,defaultURL, Hammer,
    XMLSerializer, blob, URL, location, idiagramUtil */

var version = "v. 2.02"; //Moved a lot of show/hide logic from css to js. Using lm-show/hide classes
...
// svg file found from global in html file.
var lastEmbedSrc;
...
var infoFileData = null; //contains data that is unique to this particular info file.
...
function svgloaded() {
  ...
  setTimeout(function() {
     ...
     */
    $(svgness).find("g[id]:not([id^='css']):not([id^='viewport'])")
      .each(function(index) {
        ...
        //get classes
        var classesFromDBForThisId = idiagramSvg.getDatabase(scrubbedId, "classes");
        if (classesFromDBForThisId != undefined) {
    ...
    $(".svg-pan-zoom_viewport")
      ...
      .each(function() {
        ...
        // //try putting lm-art in parent g if plain g
        // let pe = this.parentElement;
        // if ($(pe).prop('nodeName')==="g" && $(pe).attr("id") === undefined &&  $(pe).attr("class") === undefined) {
        // // $(this.parentElement).addClass("lm-art");
        //   $(this).insertAfter(this.parentElement);
        // }
      });
    // $(".svg-pan-zoom_viewport")
    //   .find(
    //     "g[mytype] *:not(g)")
    //   .each(function() {
    //     //now go through and put artwork in .sss groups that haven't been and that are not under .ooo
    //     if ($(this).parents(".ooo").length === 0 && $(this).parents(".sss").length === 0) {
    //       //now wrap this in an sss group.
    //       $(this).wrap("<g class='sss'></g>");
    ...
    $.address.change(function(e) {
      ...
      //----
      if (thisId !== null && thisId !== 0 && idGroupObject[thisId] !== undefined) {
    ...
    //TODO: See if this is redundant, since this should be fitted and centered on startup
    normalizeMapPane();
...
function getDatabaseAndInfoFile() {
  ...
  if (designerPrefs.masterDB !== undefined && designerPrefs.masterDB.length) {
    $.get("/mapdata/" + designerPrefs.masterDB, function(data, status) {
      ...
      if (status === "success" && data.length) {
        ...
        // Now get the override database
        getOverRideDatabase(masterDatabase);
      }
      else {
        getOverRideDatabase(null);
        // database = [];
  ...
  $.get(designerPrefs.infoFile, function(data, status) {
    if (status === "success") {
      ...
      console.log("infopane data Error: " + status);
    }
  });
  if (typeof designerPrefs.segmentsFile !== "undefined" && designerPrefs.segmentsFile !== undefined && designerPrefs.segmentsFile.length) {
    $.get(designerPrefs.segmentsFile, function(data, status) {
      if (status === "success") {
        ...
        // process json file
        present.processAnimatePresentate(); //note that right now we cannot handle loading multiple segmentsFile's
      }
      else {
        console.log("Error loading SegmentsFile: " + status);
      }
    });
  }
    ...
    customEventsHandler: {
      ...
      init: function(options) {
        ...
        this.hammer.on('doubletap', function(ev) {
          //options.instance.zoomIn()  commented this out because we don't want double-click to do anything except on uno's
        })
      }
...
$(function() {
  /*Important: As of jQuery 1.4, if the JSON file contains a syntax error, the request will usually f
  ail silently. Avoid frequent hand-editing of JSON data for this reason. JSON is a data-interchange 
  ...
  $.getJSON(designerprefsFileName + ".json", function(j) {
      ...
      /*
       * $(".ui-layout-pane").click(function(event) { e.stopPropagation();
       * myWidgetResizer(); });
       */
    ...
    .fail(function() {
      ...
      if (typeof svgFile !== "undefined") {
        idiagramSvg.designerPrefs = designerPrefs = {
          ...
          "zoomSensitivity": zoomSensitivity = typeof zoomSensitivity !== "undefined" ? zoomSensitivity : 5.5,
          "defaultURL": defaultURL
        };
      }
      else alert("error getting designerprefs, and no svgFile defined in html file");
...
var KEYBOARD_SHORTCUTS_HERE; //get keycode here:  http://keycode.info/
    ...
    if (ext) {
      ...
      switch (ext) {
        ...
        case "hbs": //we got a handlebars file
          ...
          if (whichPane == "infofile" || whichPane == "info") {
...
/**
 * Called after loading the handlebars template that is a form for updating the map database
 **/
function callbackLoadMapEditor(fileContents, status) {
  if (status === "success") {

    var template = Handlebars.compile(fileContents);
    mapEditorTemplate = template; //global to keep track of this
    idiagramSvg.mapEditorTemplate = template;
    ...
    data.masterChecked = "checked";
    var result = template(data);

    //Note when we load this in, this may contain a global variable called infoFileData. NOTE 2: We are currently not using infoFileData.
    //it contains data that is unique to this particular info file.
    // here is an example:
    // var infoFileData = {
    //   dbEditing : true //let's me know that this allows user to click on a uno and have it populate this form with it's row of data from the database
    ...
    if (dyn) {
      ...
      // $('.ui-layout-pane-east').scrollTop(0); //scroll to top of new content in info pane
      // createHoverEventForTaggedWords();
      // mapDbApi.setMapFormSubmitButtonEvent(); //Don't do this now. It gets done in populateMapForm.  This sets an event handler that waits for the submit button
    }
...
function purgeMatchedStatesFromParamList(parameterList) {
  ...
  for (var i = 0; i < parameterList.length; i++) {
    ...
    // hoover off
    var rfxtypes = /^(?:\+\+\+|panx|pany|zoom|panzoom|all|animate|play|stop|stopall|openall|closeall|wpane|epane|info|story|mapform|edit|map|infotext|storytext|maptext|goto|gotoz|run|move|scale|fade|rotate|masteron|masteroff)$/;
...
/**
 * Send in an array of objects containing parameters of commands (such as open, close, ) This will reverse
 * those commands. This is called from hover off.
 */
// TODO:This is nothing but ugly. Not all commands can be reversed. It is
// confusing.
/* function reverseCommands(parameterList) {
  // var newParameterList = [];
  var command;
  var groupId, groupObject;
  for (var i = 0; i < parameterList.length; i++) {
    ...
    switch (command) {
      case "on":
        groupObject.setThisToOff(true);
        break;
      case "open":
        groupObject.setThisToClose(true);
        break;
      case "off":
        groupObject.setThisToOn(true);
        break;
      case "close":
        groupObject.setThisToOpen(true);
        break;
      case "+++":
        break;
      default:
  ...
  if (svgness !== undefined) {
    ...
    if (!q || q.length < 2) {
      ...
      var keys = ["+++", "on", "off", "open", "close", "all", "animate", "play", "stop", "openall", "closeall", "stopall",
        "info", "story", "mapform", "edit", "infotext", "maptext", "storytext", "map", "goto", "gotoz", "run", "move", "scale", "fade", "rotate",
        "masteron", "masteroff"
      ];
...
function createHoverEventForTaggedWords() {
  ...
  /*
   * jQuery.fn.singleDoubleClickText = function(single_click_callback,
   * double_click_callback, timeout) { addClass(this, "clicked"); return
   * this.each(function() { var clicks = 0, self = this;
   * jQuery(this).click(function(event) { clicks++; if (clicks == 1) {
   * setTimeout(function() { if (clicks == 1) {
   * single_click_callback.call(self, event); } else {
   * double_click_callback.call(self, event); } clicks = 0; //timeout || 350); },
   * 0); } //We don't want to call anything else. We will handle text url
   * clicking here only. //TODO: See if this stops double-clicks
...
// Now loose the ddd or vvv
function stripVvvOrDdd(truncatedId) {
  var pos = truncatedId.indexOf("ddd");
  pos > -1 ? truncatedId = truncatedId.slice(3, truncatedId.length) : truncatedId;
  // get the vvv or ddd off
  pos = truncatedId.indexOf("vvv");
  // get the vvv or vvv off
  pos > -1 ? truncatedId = truncatedId.slice(3, truncatedId.length) : truncatedId;
  return truncatedId;
}
/**
 * Enter an id and field that you want back. This will remove junk from front and end of thisId and look it up
 * in database and return the value. Removes mmm and vvv from front and junk on end that is put there by AI
 */
function getDatabase(thisId, field) {
  // truncate "_" and subsequent characters
  var truncatedId = stripId(thisId);
  // get the dash stuff off this so we get this id to look like the ones in
  // the database
  // truncatedId = stripDash(thisId);
...
function deleteItemFromDatabase(thisId, field) {
  // truncate "_" and subsequent characters
  var truncatedId = stripId(thisId);
  // Now loose the ddd or vvv
  // truncatedId = stripVvvOrDdd(truncatedId);
...
function getSelectorAndResultArray(customFuncParameters) {
  ...
  var selector = shiftParams.match(/^['’`]([^’'`]+)[’'`]/); //selector[1] may look like: ".step3>.ooo, step3>.vvv>.ccc, .step3>.arclabel"
  var relative;
  if (selector !== undefined && selector !== null) {
    ...
    if (resultArray !== undefined && resultArray.length > 2) {
      ...
      /*selector[1] = selector[1].replace(/^[ ]*([^\.#])/g, function(a, b) {
...
function processCommandsInURL(q, scrollToId) {
  ...
  if (!isKeyInParameterList(q, ["+++"])) {
    // here we want to make sure out-of-place objects are put  back.
    // callAllGroups("putBackIntoPlaceHolder");

    //TODO: Why do we have this? Need better documentation
    if (!isKeyInParameterList(q, ["all"])) {
  ...
  for (var i = 0; i < q.length; i++) {
    ...
    switch (keyName) {
      ...
      case "off":
        ...
        //   }
        //   else {
        //     //removeCommandFromURL("lock", objectId, true); //take out "on" commands for this id, since it usually makes no sense to have them in the url
        //     removeDupCommandValFromUrl(keyName, objectId); //remove duplicate of this key/value from url
        //     $.address.update(); //update the url
        //     if (objectId in idGroupObject) {
        //       idGroupObject[objectId].unlockOnOff();
        //     }
        //     else {
        //       alert("Bad id in unlock command: " + objectId);
        //     }
        //   }
        ...
        // case "lock":
        //   objectId = q[i][keyName];
        //   // removeCommandFromURL("unlock", objectId, true); //take out "on" commands for this id, since it usually makes no sense to have them in the url
        //   removeDupCommandValFromUrl(keyName, objectId); //remove duplicate of this key/value from url
        //   $.address.update(); //update the url
        //   if (objectId in idGroupObject) {
        //     idGroupObject[objectId].lockOnOff();
        //   }
        //   else {
        //     alert("Bad id in lock command: " + objectId);
        //   }
        //   break;
        ...
        //   $.address.update(); //update the url
        //   if (objectId in idGroupObject) {
        //     idGroupObject[objectId].toggleLockOnOff();
        //   }
        //   else {
        //     alert("Bad id in lock command: " + objectId);
        //   }
      ...
      case "edit": //edit map database. Looks like edit=unoid. Use this so you don't have to click on an uno to edit it.
        ...
        if ($("#mapDbEditor").length === 0) //don't reload template file if we already have it. Note: someday we may want to load different map db editors
        {
          $.get("/mapDbEditor.hbs", function(unoIdToEdit, fileContents, status) {
            callbackLoadMapEditor(fileContents, status);
            mapDbApi.populateMapForm(unoIdToEdit, idiagramSvg.database);
