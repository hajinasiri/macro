...
// /**
// * From list of enhancements wish-list in SMP Spec 0.5.doc:
// * Allow creation of GIdObjects’ for DB entries without matching SVG groups
// We’re now creating UNOs that store data in the DB but do not have a matching SVG element in the SVG file.  
// Currently the code will not load the DB entries without a matching SVG <g id=”unoID”>.

// When loading the map, it's looking to match SVG UNOs to a DB IDs. If there's no SVG element to match the 
// DB element, then the DB element doesn't get loaded (i.e no GIDObject is created for that DB entry).
// We need to write a hack that will insert a dummy SVG element with an id.  The reason being, that I'm 
// creating 'UNOs' (DB entries) that just refer to, or control, other UNOs. So they don't have any artwork 
// of their own. To get them to work now, I have to go into Illustrator and create dummy artwork, which is 
// a pain.
// **/
// function insertDummyElementsForLazyDesigners(svgness, database) {
//   //for each row in the database, see if there is a missing UNO in the svg object. If there is,
//   //Create a dummy element with the id and insert it into the svg object.
//   //for each (var item in database)
//   var whereInsertDummyElements = $(svgness).find('.svg-pan-zoom_viewport')[0];
//   for (let id of database) {
//     console.log("id: " + id[0]);
//     var result = $("#" + id[0]);
//     if (result.length === 0) {
//       //insert dummy element in svg  file
//       // $(svgness).find("g[id]:not([id^='css']):not([id^='viewport'])").each(function() {  
//       //var newElement = $(svgness, ".svg-pan-zoom_viewport").prependTo('<div id=\"'+id[0]+'\"></div>');
//       $(whereInsertDummyElements).prepend('<g id=\"' + id[0] + '\"></g>');

//       //$(newElement).attr("id", id[0]);
...
function svgloaded() {
  ...
  // insertDummyElementsForLazyDesigners(svgness, database);
  ...
  setTimeout(function() {
    ...
    if (!$(svgness).hasClass("preprocessed")) {
      ...
      $(svgness).find("g[id]:not([id^='css']):not([id^='viewport'])").each(function() {
        ...
        if (!/^sss|^xxx|^vvv|^ooo/.test(scrubbedId)) {
          ...
          //deleting this because this has to be called after  insertDummyElementsForLazyDesigners(svgness, database) so we can insert stuff from map database
       ...
       **/
     // took this out because caused some side effects. insertDummyElementsForLazyDesigners(svgness, database);
      $(svgness).find("g[id]:not([id^='css']):not([id^='viewport'])").each(function() {
        var thisId = $(this).attr("id");
        ...
        if (!/^sss|^xxx|^vvv|^ooo/.test(scrubbedId)) {
          ...
          //this has to be called after  insertDummyElementsForLazyDesigners(svgness, database) so we can insert stuff from map database
          var classesFromDBForThisId = getDatabase(scrubbedId, "classes");
          if (classesFromDBForThisId != undefined && classesFromDBForThisId.length) {
            $("#" + scrubbedId).addClass(classesFromDBForThisId);
          }
    ...
    $.address.externalChange(function(e) {
       ...
       $.address.value(val);
       //idiagramSvg.PreviousUrlAddress = $.address.value(); //this may or may not 
      setTimeout(function() {
        // myClientSocket.emitCommandsForUrl($.address.value());
...
function finishCreatingInfoPanePlaceholder(unoGIdObject, longDescription) {
  ...
  unoGIdObject.longDescription = longDescription;
  if (webLinks.length) {
    unoGIdObject.longDescription += '\n\n### Further Information\n\n' + webLinks;
  }
  if (references.length) {
    unoGIdObject.longDescription += '\n\n### Bibliography\n\n' + references;
  }
  // convert markdown to html
  unoGIdObject.longDescription = converter.makeHtml(unoGIdObject.longDescription);
  unoGIdObject.longDescription = "<infopane-title>" + label + "</infopane-title>\n\n" + unoGIdObject.longDescription;
  ...
  $(divToWrap).append(unoGIdObject.longDescription);
  unoGIdObject.infoPaneData = divToWrap; //changed this from infoPane because this property is being used as something else, like hover or whatever.
  // next insert this into the info-pane. It should start out
  // invisible
  $(".information-pane").append(unoGIdObject.infoPaneData);
  createHoverEventForTaggedWords();
...
function populateInfoPane(unoGIdObject) {
  ...
  if (unoGIdObject.shouldPopulateInfoPane === "1") {
    ...
    /**
     * The following tests to see if the description should be loaded in from a file. We know that because we 
     * have "+++&fileInfo=" prepended to the longdescription text instead of text.
     * This is a hack that allows designer to replace text in longDescription with a command that populates the
     * info pane (the pane on the east side of the screen) with stuff from one of three sources, depending on the
     * command in the longDescription field, which comes from populating the longDescription field in the map database.
     * These commands are: 
     * unoInfo=unoID (in this case, we cannot use the alternative command, “info”.)
     * fileInfo=pathname/filename.ext
     * formInfo=formName
     * TODO: It is recommended that a future version of this is more robust. Another field in the map database would be
     * a drop-down list of the four options -- (1) text found in longDescription, (2) unoInfo (3) fileInfo (4) formInfo,
     * with the command parameters found in the longDescription field.
     **/

    if (/\+\+\+&unoInfo/.test(longDescription)) {
      let re = /\+\+\+&unoInfo=(.+)/i; //regular expression: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp
      let found = longDescription.match(re); //grab the uno id. found[1] contains the uno id

      var referencedUnoGIdObject = idGroupObject[found[1]]; //now this will call code below to populate the info pane with the longDescription in the object associated with id found[1]
      longDescription = getDatabase(referencedUnoGIdObject.uno, "longDescription"); //now we need to get the longDescription of the UNO id referenced in the UnoInfo command
      if (unoGIdObject.populateInfoPaneWasCalledOnce === false) {
        // unoGIdObject.populateInfoPaneWasCalledOnce = true; just always call this because it does not work.
      ...
      if (unoGIdObject.shouldPopulateInfoPane === "1") {
        ...
        if (unoGIdObject.uno !== CurrentIdOfInfoPane) {
          ...
          $("#mapDbEditor").hide();
          $(unoGIdObject.infoPaneData).addClass("show");
    ...
    else if (/\+\+\+&fileInfo/.test(longDescription)) {
      let re = /\+\+\+&fileInfo=(.+)/i; //regular expression: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp
      let found = longDescription.match(re); //grab the file name. found[1] contains the file name
      var fileName = found[1];
      if (fileName !== undefined && fileName !== null) {

        $.get(fileName, callbackLoadDynamicContent.bind(this, fileName, "fileInfo"));
    ...
    else if (/\+\+\+&formInfo/.test(longDescription)) {
      let re = /\+\+\+&formInfo=(.+)/i; //regular expression: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp
      let found = longDescription.match(re); //grab the file name. found[1] contains the file name
      var forminfo = found[1];
      if (forminfo !== undefined && forminfo !== null) {
        var unoLabel = {
          label: "" // This will load some sort of text into the form title. 
          //In this case, nothing, but I guess we still need to have it.This label only applies when this is associated with a 
          //UNO as in populateInfoPane() when loaded in via longDescription
        };
        //------------------------------------------------------------------
        ...
        $.get("/formApi/" + forminfo + "/" + getGlobal("formDB") + "s", unoLabel, function(data, status) {
          // TODO: Create better error-handling
          if (status === "success") {
            //--------------------------------------------------------
  ...
  if (!isKeyInParameterList(q, ["+++"])) {
    ...
    if (isKeyInParameterList(q, ["---"])) {
      for (var i = 0; i < q.length; i++) {
        var keyName = Object.getOwnPropertyNames(q[i])[0]; //keyName is the command name, such as "open"
  ...
  for (var i = 0; i < q.length; i++) {
    ...
    switch (keyName) {
      ...
      case "formInfo": //the form ‘formName’ must be stored in the form database specified in the map’s config.json file: "formDB": "theFormDB",
        ...
        $.address.autoUpdate(autoUpdate);
        // var filePath = forminfo; //designerPrefs.folder + "/" + forminfo;
