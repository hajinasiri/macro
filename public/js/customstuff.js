...
//
// duration is the drawing time in seconds
// numDraws is the number of times we should draw the links  - optional
// drawKind is the name of a special drawing tween           - optional
//
...
//
// Scales and fades UNO objects on open/close.
// Scaling by default is done using the center of the UNO's ooo as the transformOrigin, optional the 'toParent' parameter can set the scale origin to the parent's ooo
// If the UNO has the .partof or .connection class, the lines will be drawn.
//
...
// toParent     Optional – boolean, default = 0 (false).  If 0, the scale origin is the children's ooo, if 1 (true) the scale origin is the unoID's ooo,
...
//        alert("Could not find valid " + closePart + "-group in " + unoID);
    ...
    if(on > 0) {
        ...
        for (i = 0; i < openThing.children.length; i++) {
            unoTo = idiagramSvg.getDatabase(openThing.children[i].id, "unoTo");
...
// vvvDealy = the delay, in seconds, between the individual child UNOs within a level
//
...
function label(p) {
    // This is just the URL-command version of the setLabel function
    var params = p.fullParameterString.split(",");
    var unoID = params[0];
    var label = params[1];

    setLabel(unoID, label)
}

function setLabel(unoID, label) {

    // This was some tricky selectoring...

    var uno = $("#"+unoID);
    var theLabel = uno.children('.label')
    var theTextContent = theLabel.children('text');
    theTextContent.text(label);
    setDatabase(unoID, 'label', label)
...
function classHlt(p) {
    ...
    //remove all these commands from url
    $.address.parameter("classHlt", "");
}

function classUnhlt(p) {
    var theClasses = "";
    var classes = p.fullParameterString.split(" ");
    for(i =0; i < classes.length; i++) {
        theClasses += "." + classes[i];
    ...
    for(var i=theThings.length - 1; i >= 0; i--){
        var unoID = theThings[i].id;
        idiagramUtil.unhlt(unoID);
    }

    //remove all these commands from url
    $.address.parameter("classUnhlt", "");
}

function classTogglehlt(p) {
    var theClasses = "";
    var classes = p.fullParameterString.split(" ");
    for(i =0; i < classes.length; i++) {
        theClasses += "." + classes[i];
    ...
    for(var i=theThings.length - 1; i >= 0; i--){
        var unoID = theThings[i].id;
        idiagramUtil.togglehlt(unoID);
    }

    //remove all these commands from url
    $.address.parameter("classTogglehlt", "");
}
...
function size(p) {
    ...
    selector = $(unoID).children(".sss:not(.label)");
...
// ------------------------------------------ Slide functions ----------------------------------------------------------
//
// Used to select slides from the story pane via code.
//
...
// We have 2 video elements in in the mappane with different z-indexes to play under or over the map: ID's of undermapVideo and overmapVideo
// And separate play functions to play a video under or over the map.

function playvidUnder(p) {
    var videoFile = p.fullParameterString;
    // this command always plays the specified video source file in the map.html <video>
    var videoElement = document.getElementById('undermapVideo');
    playVid(videoElement, videoFile);
}

function playvidOver(p) {
    var videoFile = p.fullParameterString;
    // this command always plays the specified video source file in the map.html <video> element id='mapVideo'
    var videoElement = document.getElementById('overmapVideo');
    playVid(videoElement, videoFile);
}

function playVid(videoElement, videoFile){

    // In order to access the video controls of undermapVideo, we make the map pane - but not the SVG artwork elements - invisible to the pointer;
    // Be sure to reset this when your're done with video - e.g. in resetvid()
...
// Reset all videos - both overmapVideo and ubermapVideo
function resetvid(p) {
    var videoFile = p.fullParameterString;
    ...
    videoElement = document.getElementById('undermapVideo');
    videoElement.src = videoFile;
    videoElement.pause();
    videoElement.currentTime = 0;
    videoElement.style.visibility = "hidden";

    // Reset the pointer events for the map - to re-enable mouse-drag-panning
    let sMap = document.querySelector('.svgMap');
    sMap.style.pointerEvents = "auto";
}

function pausevid(p) {
    var videoFile = p.fullParameterString;
    // this command always pauses the specified video source file in the map.html <video> elements: id='overmapVideo' and/ id='undermapVideo'
...
function startWebcam(p) {
    ...
    if( video.srcObject != null){
        ...
        video = document.getElementById(webcamID);
...
var noTitle = {
    // Despite the name, it still creates a tt title
